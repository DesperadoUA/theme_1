<?php
$data = [];
$data['hide'] = carbon_get_theme_option( 'widget_2_hide' );

if(!$data['hide']) {
    $data['src'] = carbon_get_theme_option( 'widget_2_bg' );
    if(empty($data['src'])) $data['src'] = get_template_directory_uri() . '/images/default-widget.png';

    $data['title'] = carbon_get_theme_option( 'widget_2_title' );
    if(empty($data['title'])) $data['title'] = 'Игра на раздевание';

    $data['text_button'] = carbon_get_theme_option( 'widget_2_text_button' );
    if(empty($data['text_button'])) $data['text_button'] = 'Начать игру';

    $data['link_button'] = carbon_get_theme_option( 'widget_2_link_button' );
